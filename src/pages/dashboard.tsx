...
import { getInteractions } from 'apiSdk/interactions';
import { InteractionInterface } from 'interfaces/interaction';
import { getFeedbacks } from 'apiSdk/feedbacks';
import { FeedbackInterface } from 'interfaces/feedback';

import { InteractionListPage as ShowCaseList } from 'pages/interactions';

function DashboardPage() {
  const { session } = useSession();

  const interactionStat = useSWR<PaginatedInterface<InteractionInterface>>(
    () => `/interactions`,
    () => getInteractions({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Interactions',
      result: interactionStat?.data?.totalCount,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`interactions`} passHref legacyBehavior>
                  <Button
